...
public class BJ_14442_벽부수고이동하기2 {
	...
	public static void goMN(int x, int y) {
		...
		q.add(new Pos(x, y, 1, 0));//x:0,y:0,Dis:1,Broken:0 시작
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (map[xx][yy] == '1') {
					...
					if (now.broken < K && !visited[xx][yy][now.broken + 1]) {
						visited[xx][yy][now.broken + 1] = true;//한번 더 부수고
						q.add(new Pos(xx, yy, now.dis + 1, now.broken + 1));//방문
					}
				} else {// 벽이 아닌 경우
					if (!visited[xx][yy][now.broken]) {//지금까지 부순거만큼 방문한 적이 없다면
						visited[xx][yy][now.broken] = true;
						q.add(new Pos(xx, yy, now.dis + 1, now.broken));//방문
					}
				}
			}
		}
